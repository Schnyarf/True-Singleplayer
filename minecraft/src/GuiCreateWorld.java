...
public class GuiCreateWorld extends GuiScreen
{
    ...
    private void updateButtonText() // There's a JAD glitch here but I doubt there's a diff that I need to worry about - Schf
    {
    ...
    protected void actionPerformed(GuiButton par1GuiButton)
    {
        if (par1GuiButton.enabled)
        {
            ...
            else if (par1GuiButton.id == 0)
            {
                ...
                this.mc.enableSP = this.mc.useSP;
                if (this.mc.enableSP)
                {
                	this.mc.setController(var8);
                	this.mc.startWorldSSP(this.folderName, this.textboxWorldName.getText().trim(), var6);
