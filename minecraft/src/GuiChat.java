...
public class GuiChat extends GuiScreen
{
    ...
    {
        if (par1Str.length() >= 1 && !this.mc.enableSP)
        {
            this.mc.thePlayer.sendQueue.addToSendQueue(new Packet203AutoComplete(par1Str));
            this.field_73905_m = true;
        }
        
        if (this.mc.thePlayer instanceof EntityClientPlayerMP && !(this.mc.thePlayer instanceof ProvisionalEntityPlayerSP2))
        {
            EntityClientPlayerMP var3 = (EntityClientPlayerMP)this.mc.thePlayer;
            var3.sendQueue.addToSendQueue(new Packet203AutoComplete(par1Str));
            this.field_73905_m = true;
        }
        else if (par1Str.startsWith("/"))
        {
            par1Str = par1Str.substring(1);
            List var6 = this.mc.getIntegratedServer().getCommandManager().getPossibleCommands(this.mc.thePlayer, par1Str);
            String var5;

            if (var6 != null)
            {
                for (Iterator var4 = var6.iterator(); var4.hasNext(); this.field_73904_o.add(var5))
                {
                    var5 = (String)var4.next();

                    if (par1Str.length() == par2Str.length() - 1)
                    {
                        var5 = "/" + var5; // Was previously var5 = (new StringBuilder()).append("/").append(var5).toString();
                    }
                }
            }
        }
    ...
    {
        return mc.enableSP; // This may be later changed to 'true' or all-out removed
    }
}
