...
public class GuiScreen extends Gui
{
    ...
    public ArrayList<ProvisionalGuiOverlay> overlays = new ArrayList<ProvisionalGuiOverlay>();

    public GuiScreen()
    {
    	ProvisionalGuiOverlay.addOverlays(this);
    }

    public void drawScreen2(int par1, int par2, float par3)
    {
        boolean doMainDrawing = true;
        for (ProvisionalGuiOverlay overlay : overlays)
        {
        ...
        if (par3 == 0)
        {
            for (int var4 = 0; var4 < this.buttonList.size(); ++var4)
            {
                ...
                if (var5.mousePressed(this.mc, par1, par2))
                {
                    ...
                    for (ProvisionalGuiOverlay overlay : overlays)
                    {
                        var6 &= overlay.actionPerformed(this, var5);
                    }
                    if (!var6)
                    {
                        continue;
    ...
    public void setWorldAndResolution(Minecraft par1Minecraft, int par2, int par3)
    {
        ...
        this.initGui();
        for (ProvisionalGuiOverlay var1 : overlays)
        {
            var1.initGui(this.buttonList, this.width, this.height);
        }
    }
